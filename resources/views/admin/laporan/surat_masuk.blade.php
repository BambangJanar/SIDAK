<x-app-layout>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        ...
        <div class="mt-4 md:mt-0">
            <a href="{{ route('laporan.cetak.surat-masuk', ['tgl_mulai' => $tglMulai, 'tgl_sampai' => $tglSampai]) }}"
                target="_blank"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div
            ...
            <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Baru</p>
            <p class="text-3xl font-bold text-blue-600">{{ $baru }}</p>
        </div>
        <div
            class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-yellow-500 border-y border-r border-gray-100 dark:bg-gray-800 dark:border-gray-700">
            <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Diproses</p>
        ...
        <div
            ...
            <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Disetujui/Selesai</p>
            <p class="text-3xl font-bold text-emerald-600">{{ $selesai }}</p>
        </div>
    </div>
            ...
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr
                        ...
                        <th class="px-6 py-4">DARI INSTANSI</th>
                        <th class="px-6 py-4">PERIHAL</th>
                        <th class="px-6 py-4">TGL DITERIMA</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800">
                    @forelse($surat as $key => $item)
                        <tr class="text-gray-700 dark:text-gray-400 hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm">{{ $item->dari_instansi ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm">{{ Str::limit($item->perihal, 40) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-gray-500 italic">
                                Tidak ada surat masuk pada periode tanggal ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
