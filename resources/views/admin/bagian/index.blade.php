<x-app-layout>
    <div x-data="{
        isModalOpen: false,
        modalMode: 'add',
        formData: { id: '', nama_bagian: '', keterangan: '' },
    
        openAddModal() {
            this.modalMode = 'add';
            this.formData = { id: '', nama_bagian: '', keterangan: '' };
            this.isModalOpen = true;
        },
        openEditModal(item) {
            this.modalMode = 'edit';
            this.formData = { id: item.id, nama_bagian: item.nama_bagian, keterangan: item.keterangan || '' };
            this.isModalOpen = true;
        }
    }">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Master Divisi / Bagian</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola data divisi/bagian untuk penempatan
                    pegawai di instansi.</p>
            </div>
            <div class="mt-4 md:mt-0">
                <button @click="openAddModal()"
                    ...
                    <i class="fa-solid fa-plus mr-2"></i> Tambah Divisi
                </button>
            </div>
        </div>
        ...
        <div
            ...
            <form action="{{ route('bagian.index') }}" method="GET" class="flex gap-4">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama divisi/bagian..."
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-4">NAMA DIVISI / BAGIAN</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800">
                        @forelse($bagian as $key => $item)
                            <tr class="text-gray-700 dark:text-gray-400 hover:bg-gray-50 transition">
                                <td class="px-6 py-4 text-center">{{ $bagian->firstItem() + $key }}</td>
                                <td class="px-6 py-4 font-semibold text-gray-800 dark:text-gray-200">
                                    <div class="flex items-center">
                                        <div
                                            class="w-8 h-8 rounded bg-blue-100 text-blue-600 flex items-center justify-center mr-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-gray-500 italic">Data
                                    divisi/bagian belum tersedia.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div @click.away="isModalOpen = false"
                ...
                <div
                    ...
                    <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200"
                        x-text="modalMode === 'add' ? 'Tambah Divisi / Bagian' : 'Edit Divisi / Bagian'"></h3>
                ...
                <form
                    :action="modalMode === 'add' ? '{{ route('bagian.store') }}' : '{{ url('bagian') }}/' + formData.id"
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama
                            Divisi/Bagian <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_bagian" x-model="formData.nama_bagian" required
                            placeholder="Cth: Divisi IT & Jaringan"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="keterangan" x-model="formData.keterangan" rows="3"
                            placeholder="Tuliskan deskripsi singkat tugas divisi..."
                            class="w-full text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"></textarea>
                    </div>
