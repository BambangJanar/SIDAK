@props(['disabled' => false, 'name', 'rows' => 3])
...
<textarea 
    ...
    rows="{{ $rows }}"
    {!! $attributes->merge([
        'class' => "block w-full mt-1 text-sm dark:text-gray-300 dark:bg-gray-700 focus:outline-none form-textarea rounded-md shadow-sm $errorClass"
    ]) !!}
>{{ $slot }}</textarea>
