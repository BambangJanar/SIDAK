...
class PengaturanController extends Controller
{
    ...
    public function update(Request $request)
    {
        $request->validate([
            ...
            'ttd_image'              => 'nullable|image|mimes:jpeg,png,jpg|max:2048', // Tambahan untuk TTD
        ]);
        ...
        if ($request->hasFile('logo_instansi')) {
            if ($pengaturan->logo_instansi && \Illuminate\Support\Facades\File::exists(public_path('images/' . $pengaturan->logo_instansi))) {
                \Illuminate\Support\Facades\File::delete(public_path('images/' . $pengaturan->logo_instansi));
            }
        ...
        // Handle Upload Gambar Tanda Tangan (TTD)
        if ($request->hasFile('ttd_image')) {
            // Hapus TTD lama jika ada
            if ($pengaturan->ttd_image && \Illuminate\Support\Facades\File::exists(public_path('images/' . $pengaturan->ttd_image))) {
                \Illuminate\Support\Facades\File::delete(public_path('images/' . $pengaturan->ttd_image));
            }
            $file = $request->file('ttd_image');
            $filename = 'ttd_' . time() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('images'), $filename);
            $pengaturan->ttd_image = $filename;
        }
